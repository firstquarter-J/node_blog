...
// 첫 화면. db에 저장된 post 가져와서 클라이언트에 보내줌
router.get("/post", async (req, res, next) => {
    try {
      const { category } = req.query;//카테고리를 query string으로 받아온다
      const post = await Post.find({ category }).sort("-postId");//검색할 카테고리를 포함한 goods를 goodsId순으로 역정렬(마이너스)
...
// post 수정
// router.patch("/detail/:postId", async (req, res) => {
//   const { postId } = req.params
//   // console.log(postId)
